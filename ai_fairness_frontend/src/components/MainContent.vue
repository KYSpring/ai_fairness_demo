...
// --- 1. 更新 formData 来匹配后端 API ---
const formData = ref({
  api_key: '',       // <-- 新增：API Key
  api_url: '',       // 后端需要
  model_name: '',    // 后端需要
  provider_name: '', // 后端会接收
  temperature: 0.0   // 后端会接收
})
...
// --- 2. 更新验证规则 ---
const rules = {
  api_key: [{ required: true, message: 'API Key is required', type: 'error' }],
...
// --- 3. (可选) 修改 handleSaveConfig ---
// "Save Config" 按钮现在没有实际作用，因为真正的触发在 "Start" 按钮。
// 我们可以让它只做一个验证。
const handleSaveConfig = () => {
  if (!formData.value.api_key || !formData.value.api_url || !formData.value.model_name) {
    MessagePlugin.error('Please fill in all required fields (API Key, URL, Model Name)');
    return;
  }
  MessagePlugin.success({
    content: 'Configuration validated successfully',
    duration: 2000,
    style: {
      background: '#f0f9eb',
      border: '1px solid #67c23a',
...
// --- 4. 彻底修改 handleStartEvaluation 来调用 API ---
const handleStartEvaluation = async () => {
  if (isEvaluating.value) return;

  // --- AUTO GENERATE 模式 (调用后端) ---
  if (uploadMode.value === 'auto') {
    // 验证表单
    if (!formData.value.api_key || !formData.value.api_url || !formData.value.model_name) {
      MessagePlugin.error('Please fill in all required fields in Step 1 (API Key, URL, Model Name) before starting.');
      return;
    }

    isEvaluating.value = true;
    const loadingMsg = MessagePlugin.loading("正在向后端发送分析请求...");

    try {
      // 使用我们在 vite.config.ts 中设置的代理
      // 注意：Vite 代理会自动将 /api/run-analysis 转发到 http://backend:5000/api/run-analysis
      // 我们在后端的 app.py 中定义的路由也是 /api/run-analysis，所以这里不需要 rewrite
      ...
      if (response.status === 202) {
        // 202 (Accepted) 表示后端已成功接收任务
        const data = await response.json();
        MessagePlugin.success({
          content: data.message || '分析任务已成功启动！这将需要几分钟到几小时。请在后端 Docker 日志中查看进度。',
          duration: 5000 // 显示 5 秒
        });
      } else {
        // 处理其他错误，如 400, 500 等
        const errorData = await response.json();
        MessagePlugin.error(`启动失败: ${errorData.error || '未知错误'}`);
      }
    } catch (error) {
      MessagePlugin.close(loadingMsg);
      console.error('网络请求失败:', error);
      MessagePlugin.error('无法连接到后端。请确保 Docker 容器正在运行且网络已连接。');
    }

    isEvaluating.value = false; // API 调用是异步的，我们在这里结束“加载中”状态

  }
  // --- MANUAL (本地) 模式 (保持原样，使用模拟数据) ---
  else {
    isEvaluating.value = true
    progress.value = 0
    const duration = 3000
    ...
    const timer = setInterval(() => {
      ...
      if (progress.value >= 100) {
        ...
        setTimeout(() => {
          ...
          showResultDialog.value = true // 显示模拟结果
        }, 500)
      }
    }, interval)
  }
...
// Mock Part II: Bias Analysis 数据 (保持不变)
...
// Mock Part III: Imbalanced Inaccuracy Analysis 数据 (保持不变)
...
// 在组件挂载时添加resize监听器 (保持不变)
onMounted(() => {
  startScrollAnimation()
  window.addEventListener('resize', handleResize)
})

// 在组件卸载时移除resize监听器 (保持不变)
onUnmounted(() => {
  if (scrollInterval.value) {
    clearInterval(scrollInterval.value)
  }
...
<div class="upload-content">
  ...
  <t-form v-if="uploadMode === 'auto'" :data="formData" :rules="rules" class="form-content">
    ...
    <t-form-item label="API Key" name="api_key">
      <t-input v-model="formData.api_key" type="password" placeholder="Enter your API Key" />
    </t-form-item>
    ...
    <t-form-item label="Provider Name">
      <t-input v-model="formData.provider_name" placeholder="Enter Provider Name (optional)" />
    </t-form-item>
    <t-form-item label="Temperature">
      <t-input v-model="formData.temperature" placeholder="Enter Model Temperature (e.g., 0.0)" />
    </t-form-item>
    <t-button theme="primary" type="submit" class="save-config-button" @click.prevent="handleSaveConfig">Validate Config</t-button>
  </t-form>
...
>
    ...
    <div class="section-table">
      ...
      <t-table
        ...
        :columns="[
          ...
          { colKey: 'impact', title: 'Impact on Sentence Prediction (In Months)', width: '220' },
  ...
  <!-- Part III: Imbalanced Inaccuracy Analysis (保持不变) -->
  <div class="analysis-section">
    <h2 class="section-title">Part III: Imbalanced Inaccuracy Analysis</h2>

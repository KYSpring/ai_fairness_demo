...
from json_trans import process_all_json_files_in_directory
from data_process_leec_bias import process_files
import logging
import pathlib
#import pystata
import stata_setup
...
# --- 1. 动态设置 Stata 路径 ---
# ==============================================================================
# 从环境变量中读取 Stata 的安装路径
stata_path_from_env = os.getenv('STATA_PATH')

if not stata_path_from_env:
    # 如果环境变量没有设置，则根据操作系统使用默认路径
    print("警告: 环境变量 STATA_PATH 未设置。正在尝试使用操作系统默认路径。")
    if os_type == "Windows":
        stata_home = pathlib.Path(r"D:\Program Files\Stata17")
    elif os_type == "Darwin":  # "Darwin" 是 macOS 的系统名
        stata_home = pathlib.Path(r"/Applications/Stata/StataMP.app/Contents/MacOS")
    else:
        # 假设在 Docker (Linux) 中运行
        stata_home = pathlib.Path(r"/usr/local/stata17")
else:
    stata_home = pathlib.Path(stata_path_from_env)

print(f"[*] 检测到操作系统: {os_type}。Stata 路径设置为: {stata_home}")

# 检查路径是否真实有效
if not stata_home.exists():
    print(f"!!! 严重错误: Stata 路径 '{stata_home}' 不存在! 请检查您的安装或 STATA_PATH 环境变量。")
    # 在非 Docker 环境下，可以抛出错误
    if os.getenv('IS_IN_DOCKER') is None:
        raise FileNotFoundError(f"Stata 路径 '{stata_home}' 不存在。")

# 1. 把目录塞进 PATH
os.environ["PATH"] = str(stata_home) + os.pathsep + os.environ.get("PATH", "")

# 2. 预先加载 DLL（关键！只在 Windows 上执行）
if os_type == "Windows":
    dll_path = str(stata_home / "stata-mp.dll")
    try:
        ctypes.WinDLL(dll_path)
        print("✅ DLL 预加载成功")
    except (OSError, AttributeError) as e:
        # 修复：AttributeError 是针对 Linux 的, OSError 是针对 Windows 路径错误的
        print(f"❌ DLL 预加载失败: {e}。")
        if os.getenv('IS_IN_DOCKER') is None:  # 只在非 Docker 的 Windows 上才强制退出
            raise
else:
    print(f"[*] 在 {os_type} 系统上，跳过 DLL 预加载。")

# 3. 现在再初始化
try:
    stata_setup.config(stata_home, "mp")
except Exception as e:
    print(f"!!! 严重错误: stata_setup.config 初始化失败: {e}")
    print("这通常意味着 Stata 路径不正确，或者 pystata/stata_setup.py 文件已损坏。")
...
gen to_drop = 0
        ...
        if reference_group_labels:  # 有参照组生成图片
            stata.run(f"""
                ...
                coefplot, keep (0.treated 1.treated 2.treated 3.treated ) ylabel({' '.join([f'{i} "{{bf:{label}}}"' for i, label in enumerate(reference_group_labels[:-1], start=1)])} , labsize(medium) ) xlabel ({xlabel_code}, labsize(medium)) xtitle( "{{bf: Coefficient}}",size(medium)) ///
            ...
            stata.run(f"""
                capture set scheme white_tableau
                graph set window fontface "Times New Roman"
                coefplot, keep (treated) ylabel(1 "{{bf:{reference_group_params[1]}}}" , labsize(medium) tstyle(major_notick)) xlabel (, labsize(medium)) xtitle( "{{bf: Coefficient}}", size(medium)) /// 
                graphregion(margin(small)) /// 
                plotregion(margin(small)) /// 
                text(, fontface("Times")) levels(95 90 ) /// 
...
def remove_duplicate_homosexual_rows(file_path):

    print("处理victim_sexual_orientation中重复值")
    # 读取 Excel 文件
    df = pd.read_excel(file_path)
    
    # 按 ID 分组，并对每组内的 'label_value' 进行去重，仅保留一行“同性恋”
    def filter_homosexual(group):
        homosexual_rows = group[group['label_value'] == '同性恋']
        if len(homosexual_rows) > 1:
            group = group.drop(homosexual_rows.index[1:])  # 保留首个“同性恋”
        return group

    # 应用过滤函数
    filtered_df = df.groupby('ID', group_keys=False).apply(filter_homosexual)
...
def batch_run(params_list, path_base, model, json_path):
    ...
    for i,param in enumerate(params_list):
        logging.info(f"进度: {i+1}/{len(params_list)} 正在处理参数: {param}")
        if param in params_data:
            ...
            # 获取每次运行的结果
            if param == "victim_sexual_orientation":
                remove_duplicate_homosexual_rows(file_path)
...
def Pxianzhupaixu(input_json_path, output_csv_path):
    ...
    # 如果DataFrame是空的（因为没有显著结果），就直接返回一个空列表
    if df.empty:
        print("信息: 在本次分析中，没有找到任何 p < 0.1 的显著结果。")
        return []
...
def find_figure_files(output_csv_path):
    """
    根据 CSV 文件的路径，查找 figure 文件夹下与 Label Name 匹配的文件路径。

    参数:
    csv_path (str): CSV 文件的完整路径。

    返回:
    list: 按 CSV 文件顺序排列的文件路径列表（不去重）。
    """

    if not os.path.exists(output_csv_path):
        print(f"信息: {os.path.basename(output_csv_path)} 文件不存在，跳过查找图表。")
        return []  # 如果文件不存在，直接返回一个空列表，避免后续代码出错

    # 解析 figure 文件夹路径
    csv_dir = os.path.dirname(output_csv_path)  # 获取 CSV 文件所在目录
    parent_dir = os.path.dirname(csv_dir)  # 获取 CSV 文件所在目录的上级目录
    figure_dir = os.path.join(parent_dir, "figure")  # figure 文件夹路径

    if not os.path.exists(figure_dir):
        print(f"错误: 目录 {figure_dir} 不存在")
        return []
    ...
    if "Label Name" not in df.columns:
        print("错误: CSV 文件中缺少 'Label Name' 列")
        return []

    label_names = df["Label Name"].tolist()  # 按原顺序获取 Label Name
    ...
    for label in label_names:
        matched_files = [os.path.join(figure_dir, f) for f in figure_files if label in f]
        matched_files.sort()  # 排序确保一致性
        matched_file_paths.extend(matched_files)  # 追加到列表（不去重）
        
    return matched_file_paths

#统计显著标签数目
def Pnum(params_list,input_csv_path, output_csv_path, model_list=None):
# 定义分类标准（全部小写）
    SUBSTANCE_LABELS = [
    ...
    # 计算Substance和Procedural标签在params_list中的匹配数
    substance_total = sum(1 for label in SUBSTANCE_LABELS if label in params_list)
    procedural_total = sum(1 for label in PROCEDURAL_LABELS if label in params_list)
...
#计算伯努利分布
def binom_process(total_variables, model_totals, main_p_order, threshold):
    results = []
    for item in model_totals:
        k_observed = count_p_values_below_threshold(main_p_order, threshold)
        p_value = binom.sf(k_observed - 1, total_variables , threshold)
...
def count_matching_variables(total_variables, json_path):
    try:
        with open(json_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
    except FileNotFoundError:
        ...
        print(f"文件 {json_path} 不是有效的 JSON 格式")
        return 0
    ...
    for param in params_list:
        if param in data:
            keyword_params = data[param].get("keyword_params", [])
            
            if keyword_params and isinstance(keyword_params, list):
                if keyword_params[0] == 1:
                    total_variables += len(keyword_params) - 1  # 变量数等于后续字符串数
                else:
                    total_variables += 1  # 变量数为 1
    
    return total_variables

def process_data(model, path_base, json_path, output_dir, params_list):
    ...
    figure_list=find_figure_files(main_p_order)
    json_main_P_N, total_biased_labels_main = Pnum(params_list, main_p_order, main_p_num,models)

    json_inaccuracy_p0_1 = Presults_sentence_inaccuracy(result_json_path,inaccuracy_p_order)
    json_inaccuracy_P_N, total_biased_labels_inaccuracy = Pnum(params_list, inaccuracy_p_order, inaccuracy_p_num,models)

    total_variables = count_matching_variables(params_list, json_path)
    
    main_binoms0_1 = binom_process(total_variables, total_biased_labels_main, main_p_order, 0.1)
    inaccuracy_binoms0_1 = binom_process(total_variables,total_biased_labels_inaccuracy, inaccuracy_p_order, 0.1)

    main_binoms0_05 = binom_process(total_variables, total_biased_labels_main, main_p_order, 0.05)
    inaccuracy_binoms0_05 = binom_process(total_variables, total_biased_labels_inaccuracy, inaccuracy_p_order, 0.05)

    main_binoms0_01 = binom_process(total_variables, total_biased_labels_main, main_p_order, 0.01)
    inaccuracy_binoms0_01 = binom_process(total_variables, total_biased_labels_inaccuracy, inaccuracy_p_order, 0.01)
    ...
    json_robustness_lgxq_llm_full_P_N, total_biased_labels_inaccuracy = Pnum(params_list, robustness_lgxq_llm_full_p_order, robustness_lgxq_llm_full_p_num,models)

    json_robustness_llm_xingqi_p0_1 = Pwenjian(result_json_path,robustness_llm_xingqi_p_order,"results_robustness_llm_xingqi")
    json_robustness_llm_xingqi_P_N, total_biased_labels_inaccuracy = Pnum(params_list, robustness_llm_xingqi_p_order,robustness_llm_xingqi_p_num,models)
    
    json_post_2014_p0_1 = Pwenjian(result_json_path,post_2014_p_order,"results_post_2014")
    json_post_2014_P_N, total_biased_labels_inaccuracy = Pnum(params_list, post_2014_p_order,post_2014_p_num,models)
    
    json_robust_standard_errors_p0_1 = Pwenjian(result_json_path,robust_standard_errors_p_order,"results_robust_standard_errors")
    json_robust_standard_errors_P_N, total_biased_labels_inaccuracy = Pnum(params_list, robust_standard_errors_p_order,robust_standard_errors_p_num,models)
    
    json_crime_clustered_p0_1 = Pwenjian(result_json_path,crime_clustered_p_order,"results_crime_clustered")
    json_crime_clustered_P_N, total_biased_labels_inaccuracy = Pnum(params_list, crime_clustered_p_order,crime_clustered_p_num,models)
    
    json_original_main_p0_1 = Pwenjian(result_json_path,original_main_p_order,"results_original_main")
    json_original_main_P_N, total_biased_labels_inaccuracy = Pnum(params_list, original_main_p_order,original_main_p_num,models)
    
    json_special_p0_1 = Pwenjian(result_json_path,results_special_p_order,"results_special")
    json_special_P_N, total_biased_labels_inaccuracy = Pnum(params_list, results_special_p_order,results_special_p_num,models)
    ...
    return json_Consistency,json_main_p0_1, json_main_P_N,json_inaccuracy_p0_1, json_inaccuracy_P_N, df_dict,figure_list
...
def main(path_base, model,output_for_experiment_dir, params_list):
    """执行完整的处理流程，包括路径初始化和文件处理"""
    
    # 生成所需的目录
    model_dir = os.path.join(path_base, model)
    os.makedirs(model_dir, exist_ok=True)

    merged_dir = os.path.join(model_dir, "merged")
    os.makedirs(merged_dir, exist_ok=True)

    processed_dir = os.path.join(model_dir, model)
    ...
    # 构造文件路径
    data_file = os.path.join(current_dir, "有触发词数据集.xlsx")
    changed_id_file = os.path.join(current_dir, "changedID_触发词版ID_list.json")
    json_path = os.path.join(current_dir, "params.json")

    # 处理 JSON 文件
    #logging.info(f"开始处理JSON文件目录: {output_for_experiment_dir}")
    process_all_json_files_in_directory(output_for_experiment_dir, processed_dir)
    

    print("处理merged文件")
    #生成merged
    #logging.info(f"生成merged文件到目录: {merged_dir}")
    process_files(processed_dir, changed_id_file, data_file, merged_dir)

    # 运行主函数并获取返回值
    json_Consistency, json_main_p0_1, json_main_P_N, json_inaccuracy_p0_1, json_inaccuracy_P_N, df_dict,figure_list = process_data(model, path_base, json_path, output_dir, params_list)
...
if __name__ == "__main__":
    path_base = r"F:\Pycharm Projects\ai_fairness_demo-main"
    model = "deepseek_r1_32b_t0"
    output_for_experiment_dir = r"F:\Pycharm Projects\FarinessLegal\LLM-Fairness-main\LLM-Fairness-main\results\t0\deepseek_r1_32b_t0\output_for_experiment_t0"
    #params_list = ["defendant_sex", "defendant_sexual_orientation", "defender_ethnicity","defender_sex", "defender_sexual_orientation"]
    params_list = ["defendant_sex", "defendant_sexual_orientation", "defendant_ethnicity",
        "defendant_age", "defendant_education", "defendant_occupation",
        "defendant_household_registration", "defendant_nationality"]
    json_Consistency, json_main_p0_1, json_main_P_N, json_inaccuracy_p0_1, json_inaccuracy_P_N, df_dict,figure_list= main(path_base, model, output_for_experiment_dir, params_list)
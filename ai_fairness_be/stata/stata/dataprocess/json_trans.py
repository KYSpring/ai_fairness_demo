...
def clean_response(response_str, file_name, index):
    ...
    cleaned_response = cleaned_response.replace("，", ",")  # 替换中文逗号
    def safe_eval(match):
        """安全计算数学表达式，避免非法 `eval()`"""
        try:
            return str(eval(match.group(0)))
        except Exception:
            return match.group(0)  # 遇到非法格式则保持原样
    
    cleaned_response = re.sub(r'(\d+)\*(\d+)', safe_eval, cleaned_response)  # 计算 15*12
    ...
    print("\n" + "=" * 20 + f" 准备解析 (文件: {file_name}, 索引: {index}) " + "=" * 20)
